<template>
  <div ref="demo" style="width: 100%;height:100%"></div>
</template>

<script setup lang="ts">
import { ref, onMounted, getCurrentInstance } from "vue"
let demo: any = ref(null)
let myChart: any
let $echarts = getCurrentInstance()?.appContext.config.globalProperties.$echarts
onMounted(() => {
  let chart = demo.value
  myChart = $echarts.init(chart);

  let graph: any = {  //这是数据项目中一般都是获取到的
    nodes: [
  ...
  graph.nodes.forEach(function (node: any) {
    node.x = Math.round(Math.random() * 1000)  //这里是最重要的如果数据中有返回节点x,y位置这里就不用设置，如果没有这里一定要设置node.x和node.y，不然无法定位节点 也实现不了拖拽了；
    node.y = Math.round(Math.random() * 1000)
